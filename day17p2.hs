...
extend [] _ = []
extend ((d, x, y):xs) d'
    | d == d' = if x < 10 then (d, x + 1, y) : extend xs d' else extend xs d'
...
merge x v c = f $ c ++ map (\ (d, x, y) -> (d, x, y + v)) (concat x)
    where f l = reduce [minimumBy' (\ (_,_,x) (_,_,y) -> compare x y) $ filter (\ (d', x', y') -> d == d' && x == x') l | x <- [1..10], d <- [North, East, South, West]]
